...
FastAPI server that chains Gemini + ElevenLabs + Sync.so
...
from config import UPLOAD_DIR, GEMINI_API_KEY, ELEVENLABS_API_KEY, SYNC_API_KEY
from services import gemini_service, elevenlabs_service, sync_service
...
async def health_check():
    ...
    print(f"DEBUG health: GEMINI_API_KEY={repr(GEMINI_API_KEY[:8] if GEMINI_API_KEY else 'EMPTY')}")
    return {
        "status": "ok",
        "apis": {
            "gemini": bool(
            ...
            "elevenlabs": bool(
                ELEVENLABS_API_KEY and ELEVENLABS_API_KEY != "your-elevenlabs-key-here"
            ),
...
async def chat_text_only(
    ...
    try:
        ...
        ai_response = await gemini_service.generate_response(session_id, message)
        emotion, clean_text = gemini_service.parse_emotion(ai_response)
...
async def chat_full_pipeline(
    ...
    Uses all 3 APIs: Gemini → ElevenLabs → Sync.so
    """
    try:
        # 1. Get AI response from Gemini
        ai_response = await gemini_service.generate_response(session_id, message)
...
if __name__ == "__main__":
    import uvicorn
    # ASCII-only prints avoid UnicodeEncodeError on Windows (cp1252) consoles
